...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("ACC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("academiccoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ACC unit input in lnReqAmount, we generate the URI with ACC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ACC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
